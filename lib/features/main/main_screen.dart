...
import 'package:todo_/features/task/task_screen.dart';
...
class MainTaskList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final mainScreenModel = context.dependOnInheritedWidgetOfExactType<MainScreenModel>();
    final data = DataModel.maybeOf(context);
    assert(data != null && mainScreenModel != null);

    final predicate = mainScreenModel!.showCompleted ? null : (TaskModel t) => !t.completed;
                ...
                children: [
                  ...
                  ListTile(
                    title: TextButton(
                      ...
                      child: const Text("Добавить задачу", style: TextStyle(color: Color.fromARGB(255, 111, 109, 109))),
                    ),
                  ),
                ],
              );
    ...
    return Dismissible(
      ...
      child: ListTile(
        leading: Checkbox(
          ...
          activeColor: const Color.fromARGB(255, 16, 165, 21),
...
class CollapsibleAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dataModel = DataModel.maybeOf(context);
    assert(dataModel != null);
    final completed = dataModel!.getTasks().values.where((t) => t.completed).length;
...
class SwitchFilterButton extends StatefulWidget {
  const SwitchFilterButton({super.key});
...
class _SwitchFilterButtonState extends State<SwitchFilterButton> {
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      icon: Icon(
        showCompleted ? Icons.visibility_rounded : Icons.visibility_off_rounded,
        color: Colors.blue,
      ));
  }
}